...
//        empty($parameter['starttime'])  ? $starttime = '0' : $starttime = strtotime($parameter['starttime']); //开始时间
//        empty($parameter['endtime'])    ? $endtime   = strtotime("+2hours") :$endtime = strtotime($parameter['endtime']); //结束时间

        empty($parameter['starttime']) ? $parameter['starttime'] = '' :$parameter['starttime'] = strtotime($parameter['starttime']);
        empty($parameter['endtime'])   ? $parameter['endtime'] = strtotime("+2hours") :$parameter['endtime'] = strtotime($parameter['endtime']);
//                dump($parameter);
...
//            dump($pUser);
            ...
            $getlist = $this->goods
                ->alias('g')
                ->field('g.purchase_price,g.completion,g.custom_name,g.id,g.buynumber,g.name,g.goods_no,g.sell_price,g.market_price,g.cost_price,g.create_time,g.img,g.is_del,g.visit,g.store_nums,g.sort,g.sale,g.url,g.index,g.special,g.hot,g.new,g.categoryid,u.user_id,u.name uername,c.cat_id,c.cat_name')
...
//                dump($two);
        ...
        $this->assign([
            'role_id'=> $this->checkTokenSession['role_id'],
            'goodslsit' => $getlist,
...
//                dump($two);
        ...
        if($getlist){
            $res['code'] = 0;
            $res['msg'] = '請求成功';
            $res['count'] = Db('goods')->alias('g')->where($map)->count();
            $res['data'] = $getlist;
        }else{
            $res['code'] = 1;
            $res['msg'] = '暂无数据,去添加几条吧!';
...
//        $map[] = ['g.is_del','neq','1'];
        ...
        if($userInfo['pid']==0){
            ...
            $getlist = $this->goods
                ->alias('g')
                ->field('g.down_time,g.id,g.buynumber,g.name,g.goods_no,g.sell_price,g.market_price,g.cost_price,g.create_time,g.img,g.is_del,g.visit,g.store_nums,g.sort,g.sale,g.url,g.index,g.special,g.hot,g.new,g.categoryid,u.user_id,u.name uername,c.cat_id,c.cat_name')
                ->join('AdminUser u','g.userid=u.user_id','left')
                ->join('Category c','g.categoryid=c.cat_id','left')
//                ->join('goodsinfo s','g.id=s.goods_id','left')
                ->where($map)
...
//        dump($cCategoryList);
    ...
    public function add(){
        ...
        $number = strtoupper(substr(md5(rand()),1,12));
...
//        print_r($img);
        if($parameter){
            ...
            $data = [
                ...
                'custom_name'            => $parameter['custom_name'],   #自定义商品名称
    ...
    public function edit(){
        ...
        if($parameter['id']){
            ...
            $data = [
                ...
                'img'             => $img,#原图
                'name'            => $parameter['name'],   #商品名称
                'custom_name'            => $parameter['custom_name'],   #商品名称
                'like'            => $parameter['like'],   #猜你喜欢
                // 'title'           => $parameter['title'],   #标题
...
//        print_r($info);
    ...
    public function show(){
        ...
        if($account['is_del'] == 0){
            $info = Db('Goods')->where('id',$id)->update(['is_del'=>2,'down_time'=>time()]);
            if($info){
                returnResponse(200,'true',$info);
            }
            returnResponse(100,'false');
        }else{
            $info = Db('Goods')->where('id',$id)->update(['is_del'=>0,'up_time'=>time()]);
            if($info){
                returnResponse(200,'true',$info);
            }
            returnResponse(100,'false');
    ...
    public function goodsdelchange(){
        $parameter = input();
        $id = intval(trim($parameter['id']));
        $account = Db('Goods')->field('is_del')->where('id',$id)->find();
        if($account['is_del'] == 0){
    ...
    public function changeCompletion(){
        $parameter = input();
        $id = intval(trim($parameter['id']));
        $account = Db('Goods')->field('completion')->where('id',$id)->find();
        if($account['completion'] == 0){
            $info = Db('Goods')->where('id',$id)->update(['completion'=>1]);
            if($info){
                returnResponse(200,'true',$info);
            }
            returnResponse(100,'false');
        }else{
            $info = Db('Goods')->where('id',$id)->update(['completion'=>0]);
            if($info){
                returnResponse(200,'true',$info);
            }
            returnResponse(100,'false');
    ...
    public function changePurchaseStatus(){
        $parameter = input();
        $id = intval(trim($parameter['id']));
        $account = Db('order')->field('purchase_status')->where('id',$id)->find();
        if($account['purchase_status'] == 0){
            $info = Db('order')->where('id',$id)->update(['purchase_status'=>1]);
            if($info){
                returnResponse(200,'true',$info);
            }
            returnResponse(100,'false');
        }else{
            $info = Db('order')->where('id',$id)->update(['purchase_status'=>0]);
            if($info){
                returnResponse(200,'true',$info);
            }
            returnResponse(100,'false');
...
//        ->where([['user_id','eq',null],['is_templete','eq',1]])
    ...
    public function addTemplate(){
        ...
        $data = array();
        $values = array();
        if(!empty($param['color'])&&!empty($param['name'])){
            $values['goods_id'] = 0;
            $values['image'] = $param['files'];
            $values['size'] = $param['size'];
            $values['attribute_name'] = $param['attribute_name'];
            $values['sub_attribute_name'] = $param['sub_attribute_name'];
     ...
     * 批量删除模板
     */
    public function delTemplateAll(){
        $param = input('post.goods_id');
        ...
        if($param){
            ...
            $parseArr = explode(',',$param);
            array_pop($parseArr);
            foreach($parseArr as $k=>$v){
                $res = Db('goodsinfo')->where('id',$v)->delete();
            }
    ...
    public function editMoney(){
        ...
        if($id && $price && $type){
            if($type == 1){
                ...
                if($userInfo['role_id'] == 17){
                    $res = Db('goods')->where('id',$id)->update(['purchase_price'=>$price]);
                    if($res){
                        $data['code'] = 0;
                        $data['msg'] = '修改成功';
                        $data['res'] = $res;
