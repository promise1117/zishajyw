...
//        dump($times);
    ...
    public function getBuyGoodsList(){
        ...
        $res = Db::name('order')->where('status','neq',9)->where('is_del',0)->where($map)->order('id','desc')->select();
//        dump($res);
        $arr = array();
        $arr1 = array();
            ...
            foreach($start as $k=>$v){
                ...
                switch ($tempUrl){
                    case 'www.youmeistore.shop':
                        $tempUrl = '(公共)www.youmeistore.shop';
                        break;
