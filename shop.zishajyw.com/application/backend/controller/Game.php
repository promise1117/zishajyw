...
//        $this->check = Validate('Category');
    ...
    public function gameList()
    {
        ...
        if(empty($parameter['telphone'])){
            $data = Db('game')->where($map)->limit(($page-1)*$limit,$limit)->order('addtime','desc')->select();
        }else{
            $data = Db('game')->where($map)->order('addtime','desc')->select();
        }

        if($data){
            $res['code'] = 0;
            $res['msg'] = '請求成功';
            $res['count'] = count(Db('game')->where($map)->select());
            $res['data'] = $data;
        }else{
            $res['code'] = 1;
            $res['msg'] = '暂无数据,去添加几条吧!';
...
//            echo $info->getFilename();
    ...
    public function gameDel()
    {
        $id = input('post.id');
        if($id){
            $rsp = Db('game')->where('id',$id)->delete();
            $res['code'] = 0;
            $res['msg'] = '删除成功';
            $res['data'] = $rsp;
        }else{
